...
$LIVE_KEY_ID = "rzp_live_RnYADNq5F3AwSJ";
$LIVE_KEY_SECRET = "akh9meNMUDRzFEyextYyHQUR";

// ✅ Get user ID from session (no need for URL)
...
// ✅ Final card pricing logic (₹1–₹12)
switch ($cardType) {
  case "RX Medo Card":
    $amount = ($cardCategory === "Family") ? 300000 : 300000; // ₹2 family, ₹1 individual
    break;
  case "Rx Medo Top Up Card":
    $amount = 350000; // ₹3500
    break;
  case "RX Medo YoungShield Card":
    $amount = 700000; // ₹7000
    break;
  case "RX Medo CitizenCare Card":
    $amount = 1000000; // ₹10000
    break;
  case "RX Medo GuardianCare Card":
    $amount = 1500000; // ₹15000
    break;
  case "RX Medo SeniorShield Card":
    $amount = 2500000; // ₹25000
    break;
  case "RX Medo Family Card":
    $amount = 800000; // ₹8000
    break;
  case "RX Medo Family Top-Up card":
    $amount = 400000; // ₹4000
    break;
  case "RX Medo Family Secure card":
    $amount = 1500000; // ₹15000
    break;
  default:
    $amount = 100; // ₹1
...
// ✅ Override with discounted price if available from signup.php
$discountedPrice = isset($_GET['price']) ? intval($_GET['price']) : null;
if ($discountedPrice && $discountedPrice > 0) {
    $amount = $discountedPrice * 100; // Razorpay needs paise
}
...
$map = $con->prepare("INSERT INTO razorpay_orders 
  (order_id, user_id, card_type, card_category, card_subcategory, age_group, amount) 
  VALUES (?, ?, ?, ?, ?, ?, ?)");
...
<body>
    ...
    <div class="info-item">
      ...
      <span class="info-value">
        <?php echo htmlspecialchars($cardType); ?>
      </span>
    </div>

    <div class="info-item">
      <span class="info-label">Category:</span>
      <span class="info-value">Membership Card
        <?php echo htmlspecialchars($cardCategory); ?>
      </span>
    </div>
    ...
    <div class="info-item">
      ...
      <span class="info-value">
        <?php echo htmlspecialchars($cardSubcat); ?>
      </span>
    </div>
    <?php endif; ?>

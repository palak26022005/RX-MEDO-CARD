...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($stmt->execute()) {
    ...
    $original_price = 3000; // Example: later fetch dynamically based on card_type
    $final_price = $original_price;

    if(!empty($coupon_code)){
        $sql = "SELECT * FROM coupons WHERE code=? AND validity_date >= CURDATE()";
        ...
        if($resultCoupon->num_rows > 0){
            ...
            echo "<script>
                alert('✅ Coupon applied! Original Price: ₹$original_price | Discount: $discount% | Final Price: ₹$final_price');
                window.location.href='payscript.php?price=$final_price';
            </script>";
        } else {
            echo "<script>
                alert('❌ Invalid or expired coupon code. Proceeding with Original Price: ₹$original_price');
                window.location.href='payscript.php?price=$original_price';
            </script>";
        }
    } else {
        echo "<script>
